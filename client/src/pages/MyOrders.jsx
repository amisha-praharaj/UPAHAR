...
import { dummyOrders } from '../assets/alldatas';
    ...
    <div className='mt-16 pb-16'>
      ...
      {myOrders.map((order, index) => (
        <div key={index} className='border border-gray-300 rounded-lg mb-10 p-4 py-5 max-w-4xl'>
          <p className='flex justify-between md:items-center text-black-400 md:font-medium max-md:flex-col'>
            <span>OrderId : {order.id}</span>
            <span>Payment : {order.paymentType}</span>
            <span>Total Amount : {currency}{order.price}</span>
          </p>
          {order.items.map((item, index) => (
            <div key={index}
              className={`relative bg-white text-gray-500/70 ${order.items.length !== index + 1 && "border-b"
                } border-gray-300 flex flex-col md:flex-row d:items-center justify-between p-4
...
//               <div
//                 key={index}
//                 className={`relative bg-white text-gray-600 ${order.items.length !== index + 1 && "border-b"
